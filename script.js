...
function exportAsImage() {
    ...
    html2canvas(element, {
        ...
        const studentName = document.querySelector(".student-header h3")?.textContent.trim() || 'student';
...
function renderResultData(data) {
    ...
    return `
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; color: #999; font-size: 0.8rem;">
            <p>Generated by DPI Student Portal • unofficial.sdpi.pro.bd</p>
            <p>© ${new Date().getFullYear()} Oahid Towsif Shamol</p>
        </div>
    `;
}

// Result Card Download Function
function downloadResultCard() {
    const element = document.getElementById("resultPopupBody");
    const roll = document.querySelector(".result-roll")?.textContent.replace('#', '').trim() || "Result";
    ...
    showToast("Generating card... Please wait", 'info');
    ...
    html2canvas(element, {
        ...
        windowWidth: 800 // Ensure wide enough for side-by-side elements if any
    }).then(canvas => {
        const link = document.createElement('a');
        link.download = `BTEB_Result_${roll}_${Date.now()}.png`;
        link.href = canvas.toDataURL('image/png');
        link.click();
